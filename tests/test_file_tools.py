...
def mock_context():
    ...
    # Create a real VibecoreContext with a mock connection
...
class TestReadTool:
    ...
    async def test_read_existing_file(self, test_file):
    ...
    async def test_read_non_existent_file(self, temp_dir):
        """Test reading a non-existent file."""
        result = await read_file(str(temp_dir / "missing.txt"))
    ...
    async def test_read_with_offset(self, test_file):
        """Test reading with an offset."""
        result = await read_file(str(test_file), offset=3)
        lines = result.split("\n")
        assert len(lines) == 3
        assert lines[0] == "     3\tLine 3"
        assert lines[2] == "     5\tLine 5"

    async def test_read_with_limit(self, test_file):
        """Test reading with a limit."""
        result = await read_file(str(test_file), limit=2)
        lines = result.split("\n")
        assert len(lines) == 2
        assert lines[0] == "     1\tLine 1"
        assert lines[1] == "     2\tLine 2"

    async def test_read_with_offset_and_limit(self, test_file):
        """Test reading with both offset and limit."""
        result = await read_file(str(test_file), offset=2, limit=2)
        lines = result.split("\n")
        assert len(lines) == 2
...
class TestEditTool:
    ...
    async def test_edit_simple_replacement(self, test_file):
        """Test simple string replacement."""
        result = await edit_file(str(test_file), "Line 2", "Modified Line 2")
...
class TestMultiEditTool:
    ...
    async def test_multi_edit_simple(self, test_file):
        ...
        result = await multi_edit_file(str(test_file), edits)
    ...
    async def test_multi_edit_with_replace_all(self, temp_dir):
        ...
        result = await multi_edit_file(str(file_path), edits)
        assert "Successfully applied 2 edits with 4 total replacements" in result

        content = file_path.read_text()
        assert content == "FOO BAR FOO\nbaz FOO qux"

    async def test_multi_edit_sequential_dependency(self, temp_dir):
    ...
    async def test_multi_edit_string_not_found(self, test_file):
    ...
    async def test_multi_edit_same_old_new_string(self, test_file):
    ...
    async def test_multi_edit_empty_edits(self, test_file):
        """Test multi-edit with empty edits list."""
        result = await multi_edit_file(str(test_file), [])
...
class TestWriteTool:
    ...
    async def test_write_overwrite_file(self, test_file):
        """Test overwriting an existing file."""
        new_content = "Completely new content"

        result = await write_file(str(test_file), new_content)
        assert f"Successfully wrote {len(new_content)} bytes" in result

        assert test_file.read_text() == new_content

    async def test_write_to_directory(self, temp_dir):
    ...
    async def test_write_create_parent_dirs(self, temp_dir):
    ...
    async def test_write_jupyter_notebook(self, temp_dir):
        """Test that Jupyter notebooks are rejected."""
        notebook = temp_dir / "test.ipynb"

        result = await write_file(str(notebook), '{"cells": []}')
        assert "Error: For Jupyter notebooks (.ipynb files), please use the NotebookEdit tool instead" in result

    async def test_write_file_outside_cwd(self):
        """Test that files outside CWD are rejected."""
        result = await write_file("/etc/test.txt", "content")
        assert "Error: Path '/etc/test.txt' is outside the allowed directory" in result
    ...
    async def test_write_empty_file(self, temp_dir):
        ...
        result = await write_file(str(file_path), "")
